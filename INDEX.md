# Complete Content Index

Your comprehensive guide to navigating this manager's handbook.

---

## 📚 Core Manager Handbook (6 Modules)

Essential reading for all managers. Work through these sequentially.

### Foundation
1. **[Getting Started](MANAGER_HANDBOOK/00_Getting_Started.md)** (30-day onboarding plan)
   - First week priorities
   - 30-day action plan
   - Essential concepts
   - Common pitfalls

2. **[Framework Overview](MANAGER_HANDBOOK/01_Framework_Overview.md)** (Understanding the system)
   - 6 career levels explained
   - 3 evaluation pillars (Engineering, Soft Skills, Leadership)
   - Rating scale (0.0-4.0+)
   - How weights shift by level

3. **[Career Levels Guide](MANAGER_HANDBOOK/02_Career_Levels_Guide.md)** (All 6 levels overview)
   - What differentiates each level
   - Typical progression timelines
   - When to consider promotion
   - Level comparison charts

### Core Processes
4. **[One-on-One Framework](MANAGER_HANDBOOK/03_One_on_One_Framework.md)** (Effective 1:1s)
   - 4 meeting types (weekly, career, feedback, quarterly)
   - Frequency by level
   - Meeting structures
   - Career conversation integration

5. **[Goal Setting Process](MANAGER_HANDBOOK/04_Goal_Setting_Process.md)** (SMART goals)
   - Quarterly goal-setting cycles
   - SMART criteria
   - Examples by level
   - Tracking progress

6. **[Feedback Delivery](MANAGER_HANDBOOK/05_Feedback_Delivery.md)** (Giving effective feedback)
   - SBI model (Situation-Behavior-Impact)
   - Difficult conversations
   - Feedback frequency by level
   - Common mistakes

### Advanced Topics
7. **[Promotion Readiness](MANAGER_HANDBOOK/06_Promotion_Readiness.md)** (Assessment framework)
   - When to promote
   - Evidence building
   - Advocacy process
   - Calibration tips

---

## 🎯 Career Level Guides (6 Levels)

Detailed guides for each career level. Use these for level-specific coaching.

### Individual Contributor Track

1. **[Junior Developer](CAREER_LEVEL_GUIDES/01_Junior_Developer.md)** (Level 1)
   - Framework weights: 70% Engineering / 25% Soft Skills / 5% Leadership
   - Learning foundations
   - 12-24 month typical duration
   - First 90 days focus

2. **[Confirmed Developer](CAREER_LEVEL_GUIDES/02_Confirmed_Developer.md)** (Level 2)
   - Framework weights: 55% Engineering / 30% Soft Skills / 15% Leadership
   - Building independence
   - 2-4 year typical tenure
   - Feature ownership

3. **[Senior Developer](CAREER_LEVEL_GUIDES/03_Senior_Developer.md)** (Level 3)
   - Framework weights: 50% Engineering / 35% Soft Skills / 15% Leadership
   - Technical leadership
   - Mentoring others
   - Cross-team collaboration

4. **[Staff Developer](CAREER_LEVEL_GUIDES/04_Staff_Developer.md)** (Level 4)
   - Framework weights: 45% Engineering / 40% Soft Skills / 15% Leadership
   - Multi-team technical strategy
   - Staff archetypes (Tech Lead, Architect, Solver, Right Hand)
   - Organizational impact

5. **[Senior Staff Developer](CAREER_LEVEL_GUIDES/05_Senior_Staff_Developer.md)** (Level 5)
   - Framework weights: 40% Engineering / 45% Soft Skills / 15% Leadership
   - Company-level technical strategy
   - Executive partnership
   - Industry influence begins

6. **[Principal Developer](CAREER_LEVEL_GUIDES/06_Principal_Developer.md)** (Level 6)
   - Framework weights: 40% Engineering / 45% Soft Skills / 15% Leadership
   - Rare apex role (1-2 per 200 engineers)
   - Industry leadership
   - Company-wide technical vision

---

## 📖 Practical How-To Guides (6 Guides)

Step-by-step tactical guides for common situations.

1. **[How to Have Career Conversations](PRACTICAL_GUIDES/How_to_Have_Career_Conversations.md)**
   - 4 conversation types (initial, quarterly, promotion, performance)
   - Complete quarterly check-in structure
   - Example dialogues
   - Preparation checklists

2. **[How to Address Underperformance](PRACTICAL_GUIDES/How_to_Address_Underperformance.md)**
   - 4-stage framework (early concerns → coaching → PIP → termination)
   - Conversation scripts
   - Documentation requirements
   - Legal considerations

3. **[Creating Stretch Projects](PRACTICAL_GUIDES/Creating_Stretch_Projects.md)**
   - What makes a good stretch project
   - Different types of projects
   - Risk management strategies
   - Support structure needed
   - How to measure success

4. **[Peer Feedback Collection](PRACTICAL_GUIDES/Peer_Feedback_Collection.md)**
   - Setting up 360 reviews (upward, lateral, downward, self)
   - Who to ask and how many providers
   - Designing feedback questions
   - Synthesizing results
   - Sharing feedback effectively

5. **[Succession Planning](PRACTICAL_GUIDES/Succession_Planning.md)**
   - Identifying high potentials
   - Creating development plans
   - Backfill planning and knowledge transfer
   - Managing transitions
   - Retention during growth

6. **[Remote Team Development](PRACTICAL_GUIDES/Remote_Team_Development.md)**
   - Remote-specific challenges & solutions
   - Virtual 1:1 best practices
   - Keeping career visible remotely
   - Asynchronous feedback and development
   - Building remote team culture

---

## 📝 Templates (10 Ready-to-Use)

Copy, customize, and use immediately.

1. **[1-on-1 Agenda Template](TOOLS_TEMPLATES/1on1_Agenda_Template.md)**
   - Weekly check-in format
   - Career-focused 1:1
   - Feedback session
   - Quarterly review

2. **[Development Plan Template](TOOLS_TEMPLATES/Development_Plan_Template.md)**
   - Individual Development Plan (IDP)
   - Career vision
   - Skills matrix
   - 90-day goals
   - Promotion readiness tracker

3. **[Promotion Packet Template](TOOLS_TEMPLATES/Promotion_Packet_Template.md)**
   - Executive summary
   - Evidence mapping to framework
   - Peer feedback section
   - Quantitative metrics
   - Manager recommendation

4. **[Peer Feedback Request Template](TOOLS_TEMPLATES/Peer_Feedback_Template.md)**
   - 5 request types (promotion, 360 review, development, project-specific, self-collection)
   - Good vs. poor feedback examples
   - Tips for requesters and responders

5. **[Performance Improvement Plan (PIP) Template](TOOLS_TEMPLATES/PIP_Template.md)**
   - Performance concerns documentation
   - Specific improvement objectives
   - Check-in schedule
   - Support and resources
   - Consequences

6. **[Goal Setting Template](TOOLS_TEMPLATES/Goal_Setting_Template.md)**
   - SMART goal structure
   - Career goal types by category
   - Goal assessment framework
   - Quarterly check-ins and annual review
   - SMART examples (good vs. weak)

7. **[Feedback Form Template](TOOLS_TEMPLATES/Feedback_Form_Template.md)**
   - Structured 360 feedback form
   - 7 question categories (collaboration, communication, technical, leadership, growth)
   - Context-specific variations (promotion, general, behavior-specific)
   - Synthesis guide for managers
   - Works with multiple tools (Google Forms, Typeform, Culture Amp)

8. **[Promotion Evaluation Checklist](TOOLS_TEMPLATES/Promotion_Evaluation_Checklist.md)**
   - 6-phase promotion evaluation process
   - Pre-planning through onboarding
   - Evaluation criteria by level
   - Red flags and concerns
   - Go/no-go decision framework with sign-off

9. **[Interview Questions By Level](TOOLS_TEMPLATES/Interview_Questions_By_Level.md)**
   - Calibrated questions for all 5 levels (Junior through Senior Staff)
   - "What we're listening for" guidance on each question
   - Calibration table showing progression
   - Red flags by level
   - Scoring guide (1-5 scale)

10. **[Coaching Conversation Starters](TOOLS_TEMPLATES/Coaching_Conversation_Starters.md)**
    - Opening prompts by conversation topic
    - Career & growth conversations
    - Skill development coaching
    - Collaboration & communication
    - Feedback & performance
    - Difficult situations and retention

---

## 📚 Case Studies (10 Real-World Scenarios)

Learn from detailed, realistic scenarios with solutions.

### Original 5 Case Studies

1. **[The Stuck Senior Developer](CASE_STUDIES/The_Stuck_Senior_Developer.md)**
   - **Scenario:** High-performing Senior at level 3.5 years, not progressing
   - **Root cause:** Lack of clarity on Staff path, misconceptions about role
   - **Solution:** 12-month development plan with cross-team projects
   - **Outcome:** Successfully promoted to Staff
   - **Key lesson:** Most "stuck" developers lack clarity, not ability

2. **[Brilliant but Difficult](CASE_STUDIES/Brilliant_but_Difficult.md)**
   - **Scenario:** Exceptional technical skills, abrasive communication
   - **Root cause:** Lack of emotional intelligence, conflates directness with cruelty
   - **Solution:** 60-day behavior change plan with coaching
   - **Outcome:** Behavior transformation, team health restored
   - **Key lesson:** High technical competence doesn't excuse poor collaboration

3. **[The Management Question](CASE_STUDIES/The_Management_Question.md)**
   - **Scenario:** Senior IC asks about becoming a manager
   - **Root cause:** Feeling stuck, didn't understand Staff IC path existed
   - **Solution:** 3-month exploration of both IC and management paths
   - **Outcome:** Chose product-focused Staff IC path
   - **Key lesson:** Many ICs want advancement, not necessarily management

4. **[Promoted Too Early](CASE_STUDIES/Promoted_Too_Early.md)**
   - **Scenario:** Developer promoted to Senior before demonstrating readiness
   - **Root cause:** Previous manager lowered bar, left before supporting transition
   - **Solution:** 6-9 month scaffolding and skill development plan
   - **Outcome:** Grew into Senior role successfully
   - **Key lesson:** Can't undo promotion, but can help IC grow into role

5. **[The Retention Crisis](CASE_STUDIES/The_Retention_Crisis.md)**
   - **Scenario:** Top Staff engineer interviewing elsewhere, at risk of leaving
   - **Root causes:** Compensation gap, career ceiling, burnout, lack of recognition
   - **Solution:** Comprehensive retention package (comp, growth path, new projects)
   - **Outcome:** Stayed, promoted to Senior Staff as planned
   - **Key lesson:** Retention requires addressing root causes, not just money

### New 5 Case Studies

6. **[The High Performer with Low Soft Skills](CASE_STUDIES/The_High_Performer_with_Low_Soft_Skills.md)**
   - **Scenario:** Technical genius (3.5+ rating) but poor collaboration (2.0 soft skills)
   - **Root causes:** Technical trajectory bias, lack of feedback, defensive posture, role misunderstanding
   - **Solution:** 3-phase approach over 6 months (conversation → diagnosis → implementation + support)
   - **Outcome:** Soft skills improved 2.0→3.0, overall 2.8→3.1, Staff consideration approved
   - **Key lesson:** Technical brilliance without soft skills has a ceiling; respect strengths while building gaps

7. **[The Imposter Syndrome Trap](CASE_STUDIES/The_Imposter_Syndrome_Trap.md)**
   - **Scenario:** Capable Senior (objectively 3.1) paralyzed by self-doubt (self-rates 2.2)
   - **Root causes:** Attribution bias, internalized doubts, avoidance behaviors, visibility gap
   - **Solution:** 4-phase approach (reality check with data → naming pattern → evidence building → action)
   - **Outcome:** Self-rating improved 2.2→2.8, promoted to Staff track, retention improved
   - **Key lesson:** Data beats opinion; naming patterns and shared language empower high performers

8. **[The Burnout Risk](CASE_STUDIES/The_Burnout_Risk.md)**
   - **Scenario:** High performer (3.2 rating) heading toward collapse from overwork
   - **Root causes:** Reliability trap, systemic understaffing, perfectionist personality, no circuit breaker
   - **Solution:** 5-phase approach (wake-up call → assessment → workload reset → recovery → prevention)
   - **Outcome:** Normalized hours and well-being, performance maintained, systemic changes implemented
   - **Key lesson:** Burnout is a management and resource planning failure; prevent through active monitoring

9. **[The Equity Gap](CASE_STUDIES/The_Equity_Gap.md)**
   - **Scenario:** High performer discovers 20-25% pay gap to market
   - **Root causes:** Company hiring practices, gradual raises not keeping pace, market changes, no audits
   - **Solution:** Immediate substantial adjustment + systemic compensation audit + transparency
   - **Outcome:** Salary adjusted to market, systemic fixes implemented, trust partially restored
   - **Key lesson:** Fair compensation is retention issue; address gaps quickly with data and business case

10. **[The Comeback Story](CASE_STUDIES/The_Comeback_Story.md)**
    - **Scenario:** Strong performer returning from 6-month parental leave struggling to re-engage
    - **Root causes:** Changed codebase, identity shift, life circumstances (sleep deprivation), no re-entry plan
    - **Solution:** Deliberate re-entry (knowledge transfer → team integration → confidence building → momentum)
    - **Outcome:** Full re-engagement within 6 weeks, career trajectory maintained, parental leave normalized
    - **Key lesson:** Returns from leave require deliberate manager support; small investments pay off in retention

---

## ❓ FAQ Documents (37 Questions Across 15 Categories)

Quick answers to common questions, organized by topic for easy navigation.

**[Master FAQ Index](FAQS/README.md)** - Start here for all FAQ navigation

### Individual FAQ Files:

1. **[Career Path Questions](FAQS/01_Career_Path_Questions.md)** (4 questions)
   - How long at each level?
   - What if IC is stuck?
   - Can someone skip levels?
   - What if no promotion budget?

2. **[Management vs. IC Path](FAQS/02_Management_vs_IC.md)** (2 questions)
   - How to respond to management interest?
   - Can someone switch back from management?

3. **[Performance and Feedback](FAQS/03_Performance_and_Feedback.md)** (3 questions)
   - How to tell someone they're not ready?
   - What if IC disagrees with rating?
   - How often to give feedback?

4. **[Difficult Situations](FAQS/04_Difficult_Situations.md)** (4 questions)
   - What if IC is underperforming?
   - How to handle "brilliant but difficult"?
   - What if promoted too early?
   - How to respond when top performer is leaving?

5. **[Team Dynamics](FAQS/05_Team_Dynamics.md)** (2 questions)
   - How to handle conflict between ICs?
   - What if team lacks diversity?

6. **[Growth and Development](FAQS/06_Growth_and_Development.md)** (3 questions)
   - How to help IC find motivation?
   - Should IC work on what interests them?
   - How to develop leadership in ICs?

7. **[Remote/Hybrid Work](FAQS/07_Remote_Hybrid_Work.md)** (2 questions)
   - How to assess performance for remote ICs fairly?
   - How do career conversations differ for remote teams?

8. **[Compensation & Equity](FAQS/08_Compensation_and_Equity.md)** (2 questions)
   - How to handle compensation discussions during career conversations?
   - What if an IC is at the right level but underpaid?

9. **[Cross-Team & Organizational](FAQS/09_Cross_Team_Organizational.md)** (2 questions)
   - What if an IC wants to transfer to another team?
   - How to calibrate levels across teams/departments?

10. **[Specialized Situations](FAQS/10_Specialized_Situations.md)** (3 questions)
    - How to support IC returning from parental leave?
    - What if IC has performance issues due to personal circumstances?
    - How to develop ICs who are subject matter experts?

11. **[Manager Self-Development](FAQS/11_Manager_Self_Development.md)** (2 questions)
    - How do I know if I'm doing a good job developing my team?
    - What if I have too many ICs to give adequate career attention?

12. **[Technical Debt & Growth Balance](FAQS/12_Technical_Debt_and_Growth.md)** (2 questions)
    - How to balance IC's growth goals with urgent business needs?
    - What if growth opportunities require skills we don't use?

13. **[Feedback & Assessment Challenges](FAQS/13_Feedback_Assessment_Challenges.md)** (2 questions)
    - How to handle self-assessment inflation?
    - What if peer feedback contradicts my assessment?

14. **[Succession & Transitions](FAQS/14_Succession_and_Transitions.md)** (2 questions)
    - How to prepare my team for my departure?
    - What if I inherit a team with unclear/inconsistent levels?

15. **[Resources and Support](FAQS/15_Resources_and_Support.md)** (2 questions)
    - What if I'm new to managing?
    - Where to find more resources?

---

## 🗺️ Navigation by Need

### By Role

**If You're a New Manager:**
1. [Getting Started](MANAGER_HANDBOOK/00_Getting_Started.md)
2. [Framework Overview](MANAGER_HANDBOOK/01_Framework_Overview.md)
3. [One-on-One Framework](MANAGER_HANDBOOK/03_One_on_One_Framework.md)
4. [1-on-1 Template](TOOLS_TEMPLATES/1on1_Agenda_Template.md)

**If You're an Experienced Manager:**
- Go directly to relevant case studies or practical guides
- Use templates as-needed
- Reference FAQ for quick answers

**If You're Supporting a Specific Level:**
- Read that level's detailed guide
- Review adjacent levels for context
- Use development plan template

---

### By Situation

**Preparing for Career Conversation:**
1. [How to Have Career Conversations](PRACTICAL_GUIDES/How_to_Have_Career_Conversations.md)
2. [Development Plan Template](TOOLS_TEMPLATES/Development_Plan_Template.md)
3. [Relevant level guide](CAREER_LEVEL_GUIDES/)

**Considering Promotion:**
1. [Promotion Readiness Guide](MANAGER_HANDBOOK/06_Promotion_Readiness.md)
2. [Promotion Packet Template](TOOLS_TEMPLATES/Promotion_Packet_Template.md)
3. [Peer Feedback Template](TOOLS_TEMPLATES/Peer_Feedback_Template.md)

**Addressing Performance Issues:**
1. [How to Address Underperformance](PRACTICAL_GUIDES/How_to_Address_Underperformance.md)
2. [Feedback Delivery Guide](MANAGER_HANDBOOK/05_Feedback_Delivery.md)
3. [PIP Template](TOOLS_TEMPLATES/PIP_Template.md) (if needed)

**IC Feeling Stuck:**
1. [The Stuck Senior Developer](CASE_STUDIES/The_Stuck_Senior_Developer.md)
2. [How to Have Career Conversations](PRACTICAL_GUIDES/How_to_Have_Career_Conversations.md)
3. [Career Path Questions FAQ](FAQS/01_Career_Path_Questions.md)

**Difficult Behavior:**
1. [Brilliant but Difficult](CASE_STUDIES/Brilliant_but_Difficult.md)
2. [Feedback Delivery Guide](MANAGER_HANDBOOK/05_Feedback_Delivery.md)
3. [Difficult Situations FAQ](FAQS/04_Difficult_Situations.md)

**IC Asking About Management:**
1. [The Management Question](CASE_STUDIES/The_Management_Question.md)
2. [How to Have Career Conversations](PRACTICAL_GUIDES/How_to_Have_Career_Conversations.md)
3. [Management vs IC FAQ](FAQS/02_Management_vs_IC.md)

**Retention Risk:**
1. [The Retention Crisis](CASE_STUDIES/The_Retention_Crisis.md)
2. [Difficult Situations FAQ](FAQS/04_Difficult_Situations.md)
3. [Compensation & Equity FAQ](FAQS/08_Compensation_and_Equity.md)

---

## 📊 Content Statistics

**Total Documents:** 59

**By Type:**
- Core handbook modules: 6
- Career level guides: 6
- Practical guides: 6
- Templates: 10
- Case studies: 10
- FAQ files: 16 (1 index + 15 topic files with 37 questions)
- Reference docs: 4 (README, MANAGER_HANDBOOK_STRUCTURE, INDEX, SUMMARY)

**Total Content:** ~100,000+ lines / ~260 pages equivalent

**Estimated Reading Time:**
- Core handbook (all 6 modules): 3-4 hours
- All career level guides: 4-5 hours
- All practical guides: 3-4 hours (6 guides now)
- All templates: 2-3 hours (10 templates with examples)
- All case studies: 3-4 hours
- Complete handbook: 15-20 hours

**Suggested Reading Order:**
1. **Week 1:** Core handbook (6 modules)
2. **Week 2:** Relevant level guides for your ICs
3. **Week 3:** Practical guides (focus on your immediate needs)
4. **Week 4:** Templates, case studies, and FAQ
5. **Ongoing:** Reference as needed

---

## 🎯 Quick Reference

**Need to...**

**Prepare for a 1:1?**
→ [1-on-1 Template](TOOLS_TEMPLATES/1on1_Agenda_Template.md)

**Assess promotion readiness?**
→ [Promotion Readiness](MANAGER_HANDBOOK/06_Promotion_Readiness.md) + [Level Guide](CAREER_LEVEL_GUIDES/)

**Write a promotion packet?**
→ [Promotion Packet Template](TOOLS_TEMPLATES/Promotion_Packet_Template.md)

**Address underperformance?**
→ [Underperformance Guide](PRACTICAL_GUIDES/How_to_Address_Underperformance.md)

**Have a career conversation?**
→ [Career Conversations Guide](PRACTICAL_GUIDES/How_to_Have_Career_Conversations.md)

**Handle a difficult situation?**
→ [Case Studies](CASE_STUDIES/) or [FAQ Index](FAQS/README.md)

**Understand a specific level?**
→ [Career Level Guides](CAREER_LEVEL_GUIDES/)

**Create a development plan?**
→ [Development Plan Template](TOOLS_TEMPLATES/Development_Plan_Template.md)

**Get peer feedback?**
→ [Peer Feedback Template](TOOLS_TEMPLATES/Peer_Feedback_Template.md)

**Answer a specific question?**
→ [FAQ Index](FAQS/README.md)

---

## 🔄 Updates and Maintenance

**Current Version:** 1.0  
**Last Updated:** November 2025  
**Status:** Complete Phase 1-3 (MVP, Core, Polish)

**Future Additions:**
- Criteria rubrics (detailed skill breakdowns)
- Additional case studies as needed
- More FAQ categories
- Video walkthroughs (if applicable)

---

## 📬 How to Use This Index

**First Time Here?**
Start with the [README](README.md) then come back to navigate to specific content.

**Looking for Something Specific?**
Use the "Navigation by Need" or "Quick Reference" sections above.

**Want to Read Everything?**
Follow the "Suggested Reading Order" to build knowledge systematically.

**Need Quick Answers?**
Go straight to the [FAQ Index](FAQS/README.md).

---

**Pro Tip:** Bookmark this index and the FAQ index for quick access to any content in the handbook.
